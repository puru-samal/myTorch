...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        self.convolutional_layers = [
            Conv1d(num_input_channels,
                   num_channels[0], kernel_sizes[0], strides[0]),
        ...
        w_out = (((((input_width - kernel_sizes[0]) // strides[0] + 1) -
                 kernel_sizes[1]) // strides[1] + 1) - kernel_sizes[2]) // strides[2] + 1

        self.linear_layer = Linear(w_out * num_channels[2], num_linear_neurons)
    ...
    def backward(self, labels):
        ...
        for conv, act in zip(reversed(self.convolutional_layers), reversed(self.activations)):
            grad = act.backward(grad)  # TODO
